...
5. 참고 자료 부록에서 **저장소에 실제로 있는 파일을 인용하면 GitHub 링크를 건다** — 재현 스크립트, 노트북, 재현 리포트, 보존된 원본 MATLAB, 한국어 해설판 등. 형식은 `https://github.com/restful3/ml4t/blob/main/<경로>` 이고 공백은 `%20` 으로 인코딩한다. 책·표준·외부 사이트처럼 저장소에 없는 것은 링크하지 않고 서지 정보만 남긴다. 이는 3번의 상대 경로 참조 금지와 다른 규칙이다 — 공개 HTML은 `source/` 를 상대 경로로 **참조**하지 않지만, 절대 GitHub URL 로 **링크**하는 것은 권장한다.
...
- 이미지에는 대체 텍스트를 제공한다.
- 참고 자료의 GitHub 링크가 실제로 열리는지 확인한다. `validate-site.py` 가 경로 존재·인코딩·`main` 기준 여부를 검사하지만, CI 는 `docs/`·`agent-support/` 만 sparse checkout 하므로 `source/` 링크는 로컬 실행에서만 검증된다.
...
main에 push되면 GitHub Actions(`.github/workflows/pages.yml`)가 같은 검증을 실행하고, 인덱스가 오래된 경우 자동으로 재생성해 커밋한다. 이는 안전망일 뿐이며 검증 실패는 로컬에서 미리 잡는 것을 원칙으로 한다.
