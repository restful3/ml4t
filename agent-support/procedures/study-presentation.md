...
5. 참고 자료 부록에서 **저장소에 실제로 있는 파일을 인용하면 GitHub 링크를 건다** — 재현 스크립트, 노트북, 재현 리포트, 보존된 원본 MATLAB, 한국어 해설판 등. 형식은 `https://github.com/restful3/ml4t/blob/<40자리 커밋 SHA>/<경로>` 이고 공백은 `%20` 으로 인코딩한다. 책·표준·외부 사이트처럼 저장소에 없는 것은 링크하지 않고 서지 정보만 남긴다. 이는 3번의 상대 경로 참조 금지와 다른 규칙이다 — 공개 HTML은 `source/` 를 상대 경로로 **참조**하지 않지만, 절대 GitHub URL 로 **링크**하는 것은 권장한다.
6. **링크는 `main` 이 아니라 커밋 SHA 로 고정한다.** 스터디가 끝나면 학습자료가 `source/` 에서 `archive/` 로 옮겨지는데, `blob/main/source/...` 는 그 순간 404 가 되고 커밋 고정 링크는 살아남는다. 발표 시점의 버전을 정확히 인용한다는 점에서도 맞다. SHA 는 **인용하는 파일들을 이미 포함한 기존 커밋**을 쓴다(보통 작업 직전의 `git rev-parse origin/main`). 반대급부로 이후 원본 파일이 수정돼도 링크는 옛 버전을 가리키므로, 내용이 크게 바뀌면 회차 리포트의 SHA 를 갱신한다.
...
- 이미지에는 대체 텍스트를 제공한다.
- 참고 자료의 GitHub 링크가 실제로 열리는지 확인한다. `validate-site.py` 는 ref(커밋 SHA 또는 `main`)와 퍼센트 인코딩을 검사한다. 커밋 고정 링크는 불변이라 경로 존재 검사를 건너뛰므로, 새로 넣은 SHA 링크는 한 번은 직접 열어보거나 `curl` 로 200 을 확인한다.
