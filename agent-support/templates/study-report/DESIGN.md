...
- 참고 자료 부록에서 저장소에 있는 파일을 인용하면 `https://github.com/restful3/ml4t/blob/<커밋 SHA>/<경로>`(공백은 `%20`)로 링크한다. 재현 스크립트·노트북·재현 리포트·보존된 원본 MATLAB·한국어 해설판이 대상이고, 책이나 외부 표준처럼 저장소에 없는 것은 서지 정보만 남긴다. `main` 이 아니라 커밋 SHA 로 고정하는 이유는 아래 아카이브 규칙을 참고한다.
...
참고 자료의 GitHub 링크도 같은 문제를 받는다. `blob/main/source/...` 는 학습자료가 `archive/` 로 옮겨지는 순간 404 가 되므로, 링크는 `main` 대신 **커밋 SHA 로 고정**한다. 고정된 커밋은 불변이라 파일이 옮겨져도 링크가 살아 있고, 아카이브 절차에서 따로 손댈 것이 없다.
