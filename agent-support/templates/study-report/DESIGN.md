...
- 참고 자료 부록에서 저장소에 있는 파일을 인용하면 `https://github.com/restful3/ml4t/blob/main/<경로>`(공백은 `%20`)로 링크한다. 재현 스크립트·노트북·재현 리포트·보존된 원본 MATLAB·한국어 해설판이 대상이고, 책이나 외부 표준처럼 저장소에 없는 것은 서지 정보만 남긴다.
...
참고 자료의 GitHub 링크는 이 규칙의 예외가 아니라 별도 과제다. `blob/main/source/...` 는 학습자료가 `archive/` 로 옮겨지면 끊기므로 아카이브 시 함께 갱신한다. 절차는 `agent-support/procedures/archive-study.md` 를 따른다.
