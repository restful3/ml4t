#!/usr/bin/env python3
"""
Campisi et al. (2024) 논문 실증 재현 - FIXED VERSION
...
def train_and_predict_classification(pipeline, X: np.ndarray, y: np.ndarray,
    ...
    for train_idx, test_idx, should_refit in tqdm(splits, desc=desc, leave=False):
        ...
        if hasattr(cached_model, 'predict_proba'):
            ...
            # For LDA which might not have predict_proba in some cases
...
def main():
    ...
    clf_models_before = get_classification_models(use_feature_selection=False)
    clf_models_after = get_classification_models(use_feature_selection=use_feature_selection)
    ...
    for name, pipeline in reg_models_before.items():
        ...
        # Calculate probability proxy for AUC
