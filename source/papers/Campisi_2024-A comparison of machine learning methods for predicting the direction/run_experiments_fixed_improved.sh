#!/bin/bash
# =============================================================================
# Campisi 2024 논문 실증 재현 - 다중 예측 기간 실험 (FIXED + IMPROVED VERSION)
# =============================================================================
# 사용법:
#   ./run_experiments_fixed_improved.sh                        # 기본 실행 (7, 15, 30, 60일)
#   ./run_experiments_fixed_improved.sh 7 14 21                # 특정 기간만 실행
#   MAX_ITER=10 ./run_experiments_fixed_improved.sh            # 빠른 테스트 (10 iterations)
#   REFIT_FREQ=30 ./run_experiments_fixed_improved.sh          # 30일마다 재학습
#   NO_FEAT_SEL=1 ./run_experiments_fixed_improved.sh          # Feature selection 없이
#   EVAL_MODE=paper ./run_experiments_fixed_improved.sh 30     # 논문 스타일 단일 holdout
#   NON_OVERLAP=1 ./run_experiments_fixed_improved.sh 30       # 비중첩 레이블 샘플링
#   BALANCED=1 ./run_experiments_fixed_improved.sh             # 분류 class_weight 적용
#   OPT_THRESH=1 ./run_experiments_fixed_improved.sh           # 회귀 threshold 최적화
# =============================================================================
...
# 추가 옵션 (새 코드 기능)
EVAL_MODE="${EVAL_MODE:-walk-forward}"      # walk-forward / paper
NON_OVERLAP="${NON_OVERLAP:-0}"             # 1이면 비중첩 레이블 샘플링
BALANCED="${BALANCED:-1}"                   # 1이면 class_weight balanced 적용
OPT_THRESH="${OPT_THRESH:-1}"               # 1이면 회귀 threshold 최적화
...
if [ "$NON_OVERLAP" -eq 1 ]; then
    echo "Non-overlap Sampling: 활성화"
else
    echo "Non-overlap Sampling: 비활성화"
fi
if [ "$BALANCED" -eq 1 ]; then
    echo "Balanced Class Weight: 활성화"
else
    echo "Balanced Class Weight: 비활성화"
fi
if [ "$OPT_THRESH" -eq 1 ]; then
    echo "Regression Threshold Optimization: 활성화"
else
    echo "Regression Threshold Optimization: 비활성화"
fi
echo "로그 파일: $MAIN_LOG"
echo "======================================================"
echo ""
...
for days in "${FORECAST_DAYS[@]}"; do
    ...
    # 명령어 구성
    CMD="python campisi_2024_replication_fixed_improved.py --forecast-days $days"
    ...
    CMD="$CMD --evaluation-mode $EVAL_MODE"

    if [ "$NON_OVERLAP" -eq 1 ]; then
        CMD="$CMD --non-overlap-labels"
    fi

    if [ "$BALANCED" -eq 1 ]; then
        CMD="$CMD --balanced-class-weights"
    fi

    if [ "$OPT_THRESH" -eq 1 ]; then
        CMD="$CMD --optimize-threshold"
    fi
