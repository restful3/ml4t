#!/usr/bin/env python3
"""
Campisi et al. (2024) 논문 실증 재현 - PAPER MODE
...
이 스크립트는 Campisi et al. (2024)의 30일 예측 실험을 최대한
그대로 재현하기 위해 만들어졌습니다. 기본값은 다음과 같습니다.

- 학습/검증: 70% 학습, 30% 고정 holdout (논문 방식)
- 표본: 30일 간격 비중첩 샘플
- 분류: class_weight='balanced'
- 회귀: Youden의 J를 이용한 확률 임곗값 최적화
"""
...
class WalkForwardCV:
    ...
    def split(self, X: np.ndarray):
        ...
        for test_idx in range(self.train_size + self.gap, n_samples):
            ...
            if train_start >= 0 and train_end <= n_samples:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Campisi et al. (2024) 30일 예측 - Paper Mode'
    )
    parser.add_argument(
        '--forecast-days', '-f',
        type=int,
    ...
    parser.add_argument(
        ...
        default='paper',
        help='기본은 논문 holdout, 필요 시 walk-forward 선택'
    )
    parser.add_argument(
        '--allow-overlap-labels',
        action='store_true',
        help='기본(비중첩) 대신 겹치는 레이블을 허용'
    )
    parser.add_argument(
        '--disable-balanced-class-weights',
        action='store_true',
        help='분류 class_weight balanced 기본 설정을 끔'
    )
    parser.add_argument(
        '--disable-optimize-threshold',
        action='store_true',
        help='회귀 확률 임곗값 최적화를 끔'
    )
...
def main():
    ...
    non_overlap_labels = not args.allow_overlap_labels
    balanced_weights = not args.disable_balanced_class_weights
    optimize_threshold = not args.disable_optimize_threshold
