...
xlabel('Date');
ylabel('Slope=\beta(t, 1)');

figure;

plot(datetime(tday, 'ConvertFrom', 'yyyyMMdd'), beta(:, 2));
title('Kalman Filter Estimate of Intercept between EWC vs EWA');
xlabel('Date');
ylabel('Intercept=\beta(t, 2)');
figure;

yhat=NaN(size(y));
ymse=NaN(size(y));
for t=2:length(tday)
    [yhat(t), ymse(t)]=forecast(model, 1, y(1:t-1));
end

e=y-yhat; % forecast error
