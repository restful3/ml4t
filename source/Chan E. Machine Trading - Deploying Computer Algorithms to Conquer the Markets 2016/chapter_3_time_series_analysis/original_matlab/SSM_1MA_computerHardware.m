clear;
% Use only 1-dimensional state variable
...
B=NaN; % state-disturbance-loading matrix. Time invariant, undetermined values.
C=NaN(size(y, 2), 1); % Time-invariant measurement matrix, undetermined values.
...
param0=randn(1+size(y, 2)+size(y, 2)^2, 1); 
