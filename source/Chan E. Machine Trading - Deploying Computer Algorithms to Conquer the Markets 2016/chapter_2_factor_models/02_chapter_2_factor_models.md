...
For our backtest, we use mid-quotes at market close provided by the Center for Research of Security Prices (CRSP.com) survivorshipbias-free database from January 3, 2007, to December 31, 2013. We choose the midprice (half of the bid and ask prices) at the market close to represent the closing price in order to avoid effects of the widened bid-ask spread at the close. The daily market, HML, and SMB factors are from Professor French’s website mba.tuck.dartmouth.edu/ pages/faculty/ken.french/data\_library.html#Research over the same period as the price data. We divide this data into two halves, and use the first half as a trainset to estimate the factor loadings for each stock separately (i.e., in a multivariate, multiple regression).6 Using
...
The complete code can be downloaded as crossSectional\_SPX.m. The complete code can be downloaded as crossSectional\_SPX.m.
