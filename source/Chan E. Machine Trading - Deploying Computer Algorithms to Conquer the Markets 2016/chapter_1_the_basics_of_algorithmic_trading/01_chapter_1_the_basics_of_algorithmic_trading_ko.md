...
알고리즘 트레이딩 전략은 과거 또는 실시간 시장 데이터를 컴퓨터 프로그램(백테스트 또는 자동 실행 프로그램)에 입력합니다. 그러면 그 프로그램이 API를 통해 브로커에게 주문을 제출하고, 브로커로부터 주문 상태 알림을 다시 받습니다. 그림 1.1의 순서도는 이 과정을 보여 줍니다.
