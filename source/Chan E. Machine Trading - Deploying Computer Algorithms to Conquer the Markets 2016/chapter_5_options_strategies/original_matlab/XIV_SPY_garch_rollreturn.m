...
% filter. Signals by Garch prediction
...
v=load('variance_SPY', 'tday', 'deltaVF');


[~, idx1 idx2]=intersect(tday, v.tday);
assert(all(tday==tday(idx1)));

deltaVF=v.deltaVF(idx2);

isExpireDate=false(size(VX));
isExpireDate=isfinite(VX) & ~isfinite(fwdshift(1, VX));

v=load('variance_SPY', 'tday', 'deltaVF', 'trainset', 'testset');
assert(all(v.tday(idx2)==tday));
deltaVF=v.deltaVF(idx2);
...
for c=1:length(contracts)
    ...
    if (~isempty(expireIdx))
        idx=startIdx:endIdx;
        mydeltaVF=deltaVF(idx); % change in variance
        positions(idx(mydeltaVF > entryThreshold), 1)=-beta(1, idx(mydeltaVF > entryThreshold));
        positions(idx(mydeltaVF > entryThreshold), 2)=1;
