...
The references appendix must link cited artifacts that exist in this repository — reproduction scripts, notebooks, reproduction reports, preserved original MATLAB, Korean explainers — using `https://github.com/restful3/ml4t/blob/<commit sha>/<path>` with spaces percent-encoded. Pin a commit rather than `main`, because archiving a study moves material out of `source/` and would 404 every `main` link; take the sha from an existing commit that already contains those files. Books and external standards stay as bibliography text with no repository link. `validate-site.py` rejects a repository link that contains a raw space or targets a ref other than a full commit SHA or `main`, and it resolves the path in the working tree only for `main` links.
