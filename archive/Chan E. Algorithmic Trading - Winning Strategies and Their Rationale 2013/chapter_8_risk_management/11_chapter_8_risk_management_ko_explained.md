...
for t=1:length(ret_sim)
    g_cppi=g_cppi+\text{log}(1+ ret_sim(t)*D*optimalF*(1+drawdown));
    drawdown=\text{min}(0, (1+drawdown)*(1+ ret_sim(t))-1);
end
...
- `drawdown=\text{min}(0, ...)` — 낙폭을 추적합니다. 0보다 클 수 없고(그건 새 최고점이므로), 0이면 리셋됩니다.
