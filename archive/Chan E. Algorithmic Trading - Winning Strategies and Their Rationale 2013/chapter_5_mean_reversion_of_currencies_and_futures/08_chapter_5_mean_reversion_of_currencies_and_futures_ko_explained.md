...
> **용어 노트 — 공적분(cointegration):** 두 가격이 각각 따로 보면 랜덤하게 움직이지만, 둘의 차이(또는 특정 비율의 조합)는 일정 범위 안에서 머무르는 관계입니다. 줄에 묶인 두 마리 강아지처럼, 각각은 자유롭게 돌아다니지만 줄 길이 이상으로는 멀어지지 않습니다.
>
> **용어 노트 — 횡단면 평균회귀:** 여러 자산의 수익률을 비교했을 때, 최근 잘 나갔던 자산은 이후 부진하고, 뒤처졌던 자산은 따라잡는 현상입니다. "잘 나가던 것은 쉬어가고, 뒤처졌던 것은 따라잡는다"는 의미입니다.
...
> **용어 노트 — 정상(stationary):** 시계열의 평균과 분산이 시간이 지나도 변하지 않는 상태입니다. 쉽게 말해, 가격이 한없이 올라가거나 내려가지 않고 일정 범위 안에서 맴돕니다. 평균회귀 전략이 작동하려면 포트폴리오가 이 "정상" 상태여야 합니다.
...
> **용어 노트 — 헤지 비율(hedge ratio):** 두 자산을 얼마의 비율로 짝지어야 평균회귀하는 포트폴리오가 되는지를 나타내는 값입니다. 예를 들어 헤지 비율이 0.8이면, A 자산 1단위당 B 자산을 0.8단위 반대 방향으로 거래합니다. 비율이 1:1이 아닐수록 미세 조정이 필요하다는 뜻입니다.
>
> **용어 노트 — 요한센 검정(Johansen test):** 여러 시계열 사이의 공적분 관계를 찾아주는 통계 방법입니다. 단순히 "관계가 있는가?"뿐 아니라, "어떤 비율로 조합해야 가장 안정적인(정상인) 포트폴리오가 되는가?"까지 알려줍니다.
...
> **용어 노트 — Z-스코어(Z-score):** "현재 값이 평균에서 표준편차 몇 개만큼 떨어져 있는가"를 나타내는 수치입니다. Z=+2이면 평균보다 2 표준편차 위(비싸다), Z=-2이면 2 표준편차 아래(싸다)입니다. 평균회귀 전략에서는 Z가 높으면 숏(내려올 것을 기대), 낮으면 롱(올라올 것을 기대)합니다.
...
> **용어 노트 — 근월물(front month) vs 원월물(back month):** 근월물은 만기가 가장 가까운 선물 계약, 원월물은 만기가 먼 계약입니다. 예를 들어 3월 현재, 4월 만기 계약이 근월물이고 12월 만기 계약이 원월물입니다. "근(近)=가까운", "원(遠)=먼"으로 기억하세요.
...
다음으로, 롤 수익률 $\gamma$ 값을 얻기 위해 **선도 곡선(forward curve)** — 선물 가격을 만기일의 함수로 — 을 피팅할 것입니다.

> **용어 노트 — 선도 곡선(forward curve):** 같은 시점에서 만기가 서로 다른 여러 선물 계약의 가격을 만기 순서대로 이어 그린 곡선입니다. 이 곡선이 우하향(만기가 멀수록 싸다)이면 백워데이션, 우상향(만기가 멀수록 비싸다)이면 콘탱고 상태입니다. 즉, 한 번에 하루를 선택하고 피팅을 위해 5개의 연속 계약이 있는 한 가장 가까운 5개 만기의 선물 가격을 만기까지의 시간 $T$(월 단위)에 피팅합니다.
...
> **용어 노트 — ADF 검정(Augmented Dickey-Fuller test):** 시계열이 평균회귀하는지(=정상인지) 통계적으로 판단하는 검정입니다. 검정 결과의 p값이 작을수록(예: 1% 이하) "이 시계열은 평균회귀한다"고 확신할 수 있습니다. 평균회귀 전략을 적용하기 전에 반드시 거쳐야 하는 "자격 시험"이라고 생각하세요.
>
> **용어 노트 — VIX 지수:** S&P 500 옵션 가격에서 산출하는 향후 30일간의 예상 변동성 지수입니다. 시장이 불안하면 급등해서 "공포 지수"라는 별명이 있습니다. VIX 자체는 거래할 수 없고, VX 선물을 통해 간접적으로 거래합니다.
...
> **쉬운 설명 — 왜 현물 가격이 사라지는가?**
>
> 식 5.7에서 두 계약의 로그 가격 차이를 구하면:
>
> $$\log F(t, T_2) - \log F(t, T_1) = [\log S(t) + \gamma(t - T_2)] - [\log S(t) + \gamma(t - T_1)] = \gamma(T_1 - T_2)$$
>
> 현물 가격 $S(t)$가 **완전히 상쇄** 됩니다. 남는 것은 $\gamma \times (\text{만기 차이})$뿐입니다.
>
> 만기 차이 $(T_1 - T_2)$는 고정값(예: 항상 12개월)이므로, 캘린더 스프레드는 **순전히 $\gamma$(롤 수익률)에 의해서만** 움직입니다.
>
> 따라서 질문이 바뀝니다:
>
> $$\text{캘린더 스프레드가 평균 회귀하는가?} \;\Longleftrightarrow\; \gamma\text{가 평균 회귀하는가?}$$
>
> 보통 선물의 **가격 자체**는 평균 회귀하지 않습니다 (원유가 80달러에서 출발해서 다시 80달러로 돌아온다는 보장 없음). 하지만 캘린더 스프레드를 쓰면:
>
> - 현물 가격 변동은 **상쇄**됨 (롱/숏이 같은 상품이니까)
> - 남는 건 **$\gamma$의 변동**뿐
> - $\gamma$는 평균 회귀 → 거래 가능
>
> | | 1월 | 6월 | 12월 | 성격 |
> | --- | --- | --- | --- | --- |
> | 원유 가격 | \$80 | \$120 | \$60 | 추세 — 예측 어려움 |
> | 캘린더 스프레드($\gamma$) | +5% | −3% | +5% | **평균 회귀 — 거래 가능!** |
...
> **용어 노트 — OU 과정(Ornstein-Uhlenbeck process):** 평균회귀를 수학적으로 모델링하는 확률 과정입니다. "고무줄에 매달린 공"처럼, 평균에서 멀어질수록 되돌아가는 힘이 강해지는 모형입니다. 반감기는 이 되돌아가는 속도를 측정하며, 반감기가 짧을수록 빠르게 평균으로 복귀합니다.
...
> **용어 노트 — 레짐(regime):** 시장이 일정 기간 동안 보이는 구조적 행동 패턴입니다. "저변동성 + 완만한 상승" 시기와 "고변동성 + 급격한 등락" 시기는 서로 다른 레짐입니다. 레짐이 바뀌면 이전에 잘 작동하던 전략이 갑자기 실패할 수 있으므로, 어떤 레짐에 있는지 파악하는 것이 중요합니다.
...
> **용어 노트 — 볼린저 밴드(Bollinger Band):** 이동평균선을 중심으로 위아래에 표준편차 밴드를 그린 기술적 지표입니다. 가격이 상단 밴드를 터치하면 "과매수"(너무 올랐으니 숏 신호), 하단 밴드를 터치하면 "과매도"(너무 내렸으니 롱 신호)로 해석합니다.
