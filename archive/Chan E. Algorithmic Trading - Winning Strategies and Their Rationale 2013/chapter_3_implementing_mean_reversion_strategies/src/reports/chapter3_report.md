...
| 개념 | 수식 | 의미 |
|------|------|------|
| **가격 스프레드** | `y = y₁ - h · y₂` | 고정 주식 수 포트폴리오 |
| **로그 가격 스프레드** | `log(q) = h₁·log(y₁) + h₂·log(y₂)` | 고정 자본 가중치 포트폴리오 |
| **볼린저 밴드** | 진입: `abs(Z) > Z_entry`, 청산: `abs(Z) < Z_exit` | 이산적 진입/청산 |
| **칼만 필터** | `β̂ₜ = β̂ₜ₋₁ + Kₜ · eₜ` | 동적 헤지 비율 추정 |
...
| 유형 | 수식 | 특징 |
|------|------|------|
| **가격 스프레드** | `y = USO - β · GLD` | 고정 주식 수, 동적 헤지 비율 적용 |
| **로그 가격 스프레드** | `y = log(USO) - β · log(GLD)` | 고정 자본 가중치, 리밸런싱 필요 |
...
가격이 L₁으로 하락 후, 확률 p로 L₂ < L₁까지 추가 하락한 뒤 F로 회귀한다고 가정하면:
...
전환 확률 `p̂ = (F - L₁)/(F - L₂)`를 기준으로, `p < p̂`이면 L₁ 올인이 최적, `p > p̂`이면 L₂ 올인이 최적입니다. **평균 매입이 최적인 경우는 없습니다.**
...
- **측정 방정식**: `y(t) = x(t)·β(t) + ε(t)` — 관측값 y를 숨겨진 상태 β로 설명
- **상태 전이**: `β(t) = β(t-1) + ω(t-1)` — 헤지 비율 β가 랜덤워크로 진화
- **상태 업데이트**: `β̂(t|t) = β̂(t|t-1) + K(t)·e(t)` — 예측 오차 e(t)를 칼만 이득 K(t)로 보정
