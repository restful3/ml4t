...
In the references appendix, link every cited artifact that actually exists in this repository — reproduction scripts, notebooks, reproduction reports, preserved original MATLAB, Korean explainers — as `https://github.com/restful3/ml4t/blob/main/<path>` with spaces percent-encoded. Leave books and external standards as bibliography text. `validate-site.py` fails on a link whose path is missing, unencoded, or pinned off `main`.
