...
In the references appendix, link every cited artifact that actually exists in this repository — reproduction scripts, notebooks, reproduction reports, preserved original MATLAB, Korean explainers — as `https://github.com/restful3/ml4t/blob/<commit sha>/<path>` with spaces percent-encoded. Pin a commit rather than `main`: archiving a study moves material out of `source/`, which would 404 every `main` link, and the pinned commit also cites the exact version the session used. Take the sha from an existing commit that already contains those files. Leave books and external standards as bibliography text. `validate-site.py` fails on a link that is unencoded or on a ref other than a full sha or `main`.
